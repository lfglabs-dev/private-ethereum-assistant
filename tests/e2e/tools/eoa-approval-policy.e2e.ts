...
function createApprovalPolicyTools(nativeThreshold: string) {
  const runtimeConfig = createE2ERuntimeConfig(ARBITRUM_CONFIG);
...
describe("EOA approval policy E2E", () => {
  test("below-threshold sends keep the normal confirmation flow", async () => {
    const tools = createApprovalPolicyTools("1");
